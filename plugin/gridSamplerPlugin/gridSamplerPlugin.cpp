    ...
    : mLayerName(name)
{
    ...
    mInterpolationMode = GridSamplerInterpolation::Bilinear;
    // mAlignCorners = true;
    ASSERT(d == a + sizeof(size_t) * 5 + sizeof(bool) + sizeof(DataType));
...
GridSamplerPlugin::GridSamplerPlugin(const std::string name,
                                     ...
                                     bool alignCorners)
    : mLayerName(name), mInterpolationMode(interpolationMode), mPaddingMode(paddingMode), mAlignCorners(alignCorners)
{
}
...
GridSamplerPlugin::GridSamplerPlugin(const std::string name, int inputChannel, int inputHeight,
    ...
    : mLayerName(name), mInputChannel(inputChannel), mInputHeight(inputHeight), mInputWidth(inputWidth), mGridHeight(gridHeight), mGridWidth(gridWidth), mInterpolationMode(interpolationMode), mPaddingMode(paddingMode), mAlignCorners(alignCorners), mType(type)
{
}
...
{
    auto plugin = new GridSamplerPlugin(mLayerName, mInputChannel, mInputHeight, mInputWidth,
...
IPluginV2 *GridSamplerPluginCreator::createPlugin(const char *name, const PluginFieldCollection *fc)
{
    ...
    auto plugin = new GridSamplerPlugin(name, static_cast<GridSamplerInterpolation>(interpolationMode), static_cast<GridSamplerPadding>(paddingMode), static_cast<bool>(alignCorners));
    plugin->setPluginNamespace(mNamespace.c_str());
    return plugin;
}
